...
export class WriterService {
  ...
  private async generateModelComparisonGuideDraft(params: {
    ...
    const sourceMaterial = {
      ...
      officialSources: sourceContext?.officialSources ?? [],
      officialSourcesProvided: Boolean(sourceContext?.officialSources && sourceContext.officialSources.length > 0),
      verificationStatus: sourceContext?.verificationStatus,
      riskLevel: sourceContext?.riskLevel,
    ...
    const baseMessages: WriterCompletionInput["messages"] = [
      {
        ...
        content: [
          ...
          "Avoid repeating labels such as 확인 필요, 단정 불가, 공식 확인 전, 검토 필요 in the article body.",
          "Use natural personal-judgment wording instead: 아직 그렇게 보기엔 이릅니다, 저는 여기서 결론을 서두르지 않는 편이 낫다고 봅니다, 지금은 중단 확정보다는 확인해야 할 신호에 가깝습니다.",
          "If official sources are missing, keep the claim cautious in natural prose and do not assert shutdown or availability changes as fact.",
          "GitHub Issues are reinforcement signals only, not official announcements.",
      ...
      },
      {
        ...
        content: JSON.stringify({
          ...
          requiredNaturalPhrases: [
            "아직 그렇게 보기엔 이릅니다.",
            "저는 여기서 결론을 서두르지 않는 편이 낫다고 봅니다.",
            "지금은 중단 확정보다는 확인해야 할 신호에 가깝습니다.",
            "제 기준으로는 바로 갈아타기보다 작업별 의존도를 먼저 보는 쪽이 맞습니다.",
            "실제 판단은 내 계정과 작업 환경에서 확인한 뒤에 하는 편이 안전합니다.",
          ],
          requiredContent: [
            ...
            "실패 비용 기준",
            "작업별 예시",
            "Opus → Sonnet → Opus workflow",
            "모델 선택 전 확인할 설정 항목",
            "비용/속도/검증 가능성 관점",
            "내 환경에서 직접 확인할 것 체크리스트",
            "rumor/shutdown discussion only in section 4 or later",
          ],
          forbiddenExamples: [
            "DB model에 GenerationLog 필드를 추가한다.",
            ...
            "verificationStatus가 needs_manual_review이면...",
            "if 공식 확인이 나온다, then...",
            "현재 이 글에서 공식 자료를 근거로 단정할 수는 없습니다.",
    ...
    const fallback: OutlineResult = {
      ...
      outline: isModelComparisonGuide
        ? [
            ...
            "## Opus와 Sonnet을 나누는 핵심 기준 3가지",
            "- 왜 필요한가: 실패 비용, 검증 가능성, 반복 속도 같은 선택 기준을 먼저 세워야 중단설에도 글의 중심이 흔들리지 않는다.",
            "## 작업별 추천 기준",
            "- 왜 필요한가: 독자가 자기 작업을 기준으로 모델을 고를 수 있어야 한다.",
            "## Opus 중단설은 모델 선택 기준과 분리해서 봐야 합니다",
            "- 왜 필요한가: 커뮤니티/GitHub 이슈는 조기 신호이지 공식 발표가 아니므로 모델 선택 기준과 사실 확인을 분리해야 한다.",
            "## 중단설 전에 확인할 체크리스트",
                    ...
                    ...(isCommunityRumorWatch
                      ...
                      : isModelComparisonGuide
                        ? [
                            ...
                            "Opus와 Sonnet은 실패 비용, 검증 가능성, 반복 속도 기준으로 나눈다",
                ...
                [
                  ...
                  ...(isModelComparisonGuide
                    ? [
                        ...
                        "In the review report you may use 검수 terms, but flag if the reader-facing draft contains defensive audit phrases such as 현재 이 글에서 공식 자료를 근거로, 단정할 수는 없습니다, 현재 입력 기준, or 공식 확인된 자료는 없습니다.",
                        "Check that safety wording is expressed as personal judgment and natural blog prose, not as generator validation text.",
                  ...
                  mustFlag: [
                    ...
                    ...(isModelComparisonGuide
                      ? [
                          ...
                          "defensive audit phrases left in reader-facing body",
                          "missing author-view sentence in each major section",
                          "English if/then conditional sentences in normal prose",
                        ]
                      : []),
                  ],
              ...
              content:
                [
                  ...
                  ...(isCommunityRumorWatch
                    ...
                    : isModelComparisonGuide
                      ? [
                          "For model_comparison_guide with rumor_safety_overlay, prioritize tags: Claude Code, Claude Opus, Claude Sonnet, Opus Sonnet 차이, Claude Code 모델 선택, Claude Code Opus, Opus 중단설, AI 코딩 도구, 모델 선택 기준, Anthropic.",
